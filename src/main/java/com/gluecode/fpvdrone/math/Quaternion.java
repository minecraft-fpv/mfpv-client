package com.gluecode.fpvdrone.math;
...
public class Quaternion {
  ...
  public float angle() {
    return (float) Math.acos(this.w) * 2F;
  }
  
  public float getYaw() {
    Quaternion q = this;
    // yaw (z-axis rotation) -> y
    //    double siny_cosp = 2 * (q.w * q.z + q.x * q.y);
    //    double cosy_cosp = 1 - 2 * (q.y * q.y + q.z * q.z);
    double siny_cosp = 2 * (q.w * q.y + q.z * q.x);
    double cosy_cosp = 1 - 2 * (q.x * q.x + q.y * q.y);
    return (float) (Math.atan2(siny_cosp, cosy_cosp) * 180 / Math.PI);
  }
  
  public float getPitch() {
    Quaternion q = this;
    // pitch (y-axis rotation) -> x
    //    double sinp = 2 * (q.w * q.y - q.z * q.x);
    double sinp = 2 * (q.w * q.x - q.y * q.z);
    if (Math.abs(sinp) >= 1)
      return (float) (Math.copySign(Math.PI / 2, sinp) * 180 /
                      Math.PI); // use 90 degrees if out of range
    else
      return (float) (Math.asin(sinp) * 180 / Math.PI);
  }
  
  public float getRoll() {
    Quaternion q = this;
    // roll (x-axis rotation) -> z
    //    double sinr_cosp = 2 * (q.w * q.x + q.y * q.z);
    //    double cosr_cosp = 1 - 2 * (q.x * q.x + q.y * q.y);
    double sinr_cosp = 2 * (q.w * q.z + q.x * q.y);
    double cosr_cosp = 1 - 2 * (q.z * q.z + q.x * q.x);
    return (float) (Math.atan2(sinr_cosp, cosr_cosp) * 180 / Math.PI);
  }
  ...
  public static Quaternion slerp(
    ...
    double s0 = (Math.cos(thetaT) - cosTheta0 * Math.sin(thetaT) / Math.sin(
      theta0));
    double s1 = (Math.sin(thetaT) / Math.sin(theta0));
    // result = s0*start + s1*end
    Quaternion result =
      Quaternion.add(
        orientation0.scaled((float) s0),
        orientation1.scaled((float) s1)
      );
   ...
   */
  public static Quaternion rotationBetweenVectors(
    Vector3 start,
    Vector3 end
  ) {
    ...
    if (cosTheta < -1.0f + 0.001f) {
      ...
      if (rotationAxis.lengthSquared() <
          0.01f) { // bad luck, they were parallel, try again!
  ...
  public static Quaternion lookRotation(
    ...
    Vector3 newUp = Quaternion.rotateVector(
      rotateForwardToDesiredForward,
      Vector3.up()
    );
    Quaternion rotateNewUpToUpwards = rotationBetweenVectors(
      newUp,
      desiredUpInWorld
    );
    
    return Quaternion.multiply(
      rotateNewUpToUpwards,
      rotateForwardToDesiredForward
    );
  }
